...
var creatorList = ['@HilmySakti','@Zahir','@Zeeone Ofc'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6281217779427, 10k perbulan santuy aja murah kok'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'HilmySakti') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'SagiriBot') return res.json(loghandler.invalidKey)
...
router.get('/textmaker', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'HilmySakti'lllllll) return res.json(loghandler.invalidKey)
